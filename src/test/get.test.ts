...
describe('Tests for testmocker.json - GET', () => {
  ...
  it('[GET] - Should return 404', async () => {
    await request(projectBasePath).get('/unknown').expect(404);
  });
  ...
  it('[GET - params] - Should return 200 with wrong parameters when fallback option available - JSON response', async () => {
    await request(projectBasePath).get('/object?parameter=value_is_not_correct').expect(200);
  });
  ...
  it('[GET - params] - Should return 200 with wrong parameters when fallback option available - raw response', async () => {
    await request(projectBasePath).get('/string?parameter=value_is_not_correct').expect(200);
  });
  it('[GET - params] - Should return 404', async () => {
    await request(projectBasePath).get('/unknown?param=value').expect(404);
  });
  it('[GET - no project value] - Should return 404', async () => {
    await request(basePathWithoutProjectValue).get('/endpoint').expect(404);
  });
  it('[GET - project without endpoints] - Should return 404', async () => {
    await request(projectWithoutEndpointsBasePath).get('/endpoint').expect(404);
  });
});
